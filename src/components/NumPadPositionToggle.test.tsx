...
describe("NumPadPositionToggle", () => {
  it("renders three position options", () => {
    render(<NumPadPositionToggle position="bottom" onChange={jest.fn()} />);
    const radios = screen.getAllByRole("radio");
    expect(radios).toHaveLength(3);
  });

  it("marks the current position as checked", () => {
    render(<NumPadPositionToggle position="left" onChange={jest.fn()} />);
  ...
  it("updates checked state when position prop changes", () => {
    const { rerender } = render(
      <NumPadPositionToggle position="bottom" onChange={jest.fn()} />,
    );
    expect(screen.getByLabelText("Pad bottom")).toHaveAttribute(
      "aria-checked",
      "true",
    ...
    rerender(<NumPadPositionToggle position="right" onChange={jest.fn()} />);
    expect(screen.getByLabelText("Pad right")).toHaveAttribute(
      "aria-checked",
      "true",
    );
